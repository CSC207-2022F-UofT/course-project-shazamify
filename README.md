...
Run the main method in ShazamifyUI
...
Two tests are written: saving a song object and finding a song based on name. 

## SongDownloader/YTdlp Notes
Inside the SongDownloader class is a main method, which at the time of writing only runs the `moveToDatabase()` method. This iterates over the target directory and looks for .json files. These files were generated by YTdlp and contain the data necessary for initalizing the Song objects in our database.

Two tests are written: creating a Song object from a json file and downloading a .mp3 file with SongDownloader. The json file test reads from one the json files in the repo and verifies that the attributes match. The download test is recommended not to be run as it requires YTdlp to be installed on the computer, it simply verifies the existence of the file when run.
