...
Install Docker (if on Windows follow this link https://docs.docker.com/desktop/install/windows-install/). You will most likely have to follow the WSL-2 installation.
...
Run the following command in a terminal in IntelliJ, this will create the docker container.
...
Once the docker container is initialized the main method in SongDownloader should be run. The main method calls the `moveToDatabase()` method in SongDownloader which creates the Song entities based on the appropriate information in the json files in `src/resources/songs`. Finally they're stored inside the database for our program to use.
...
The DAO design pattern acts as an API for the database and implements CRUD operations (Create, Read, Update, Delete). Originally there was a single DAO class that had both read and write operations to the database. In order to adhere to the Single Responsibility Principle, two separate interfaces and classes were created. One for input and the other for output.
...
Added GitHub action to start up MongoDB server on default port so DAO tests can run on build. Look into how to ignore certain tests.

## Code coverage
This project utilised Junit4. For sake of regularity, any class ending with Test is run by the test runner. There are a few classes in the test package which are exploratory tests. Here is the following code coverage as generated by Jacoco. Nearly the entire Entity, ABR, and DS layer are covered. A lot of testing of the Framework layer was done manually instead of a test runner.

![Jacoco code coverage report](https://user-images.githubusercontent.com/69766438/206624657-ebb30244-2203-41bf-9816-b73fba002395.PNG)
