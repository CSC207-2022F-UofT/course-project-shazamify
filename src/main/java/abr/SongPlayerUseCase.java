...
public class SongPlayerUseCase {

    public static final int NUM_INTERVALS = 100;

    private static SongPlayerUseCase instance;
     ...
     * @return instance
     */
    public static SongPlayerUseCase getInstance() {
        if (instance == null) {instance = new SongPlayerUseCase();}
        return instance;
    }
...
//            stream = AudioSystem.getAudioInputStream(song.getFile());
    ...
    private void initTimer(Song song) {
        int timerDelay = (int) (song.getDuration() * 1000) / SongPlayerUseCase.NUM_INTERVALS;
        // Create timer
        timer = new Timer(timerDelay, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                int progress = SongPlayerViewModel.getProgress();
                if ( progress == SongPlayerUseCase.NUM_INTERVALS ) {
                    timer.stop();
                }
                else {
